...
About 10% of US voters have surnames that do not appear in the Census surname list. Standard BISG has no name-specific information for these voters and falls back on a name-independent imputation (the column means of the matched-surname probabilities), which gives every unmatched name the same prediction. The `eBISG` model fills this gap: it embeds each unmatched surname with a pre-trained sentence-transformer (E5-Large by default), then runs a small neural network trained on Census 2020 data to predict race-conditional probabilities for that specific name. For names that *are* in the Census list, predictions are identical to standard BISG.

`eBISG` is opt-in. It depends on Python with `sentence-transformers` and `torch`; the first call additionally downloads about 17 MB of pre-trained MLP weights from the package's GitHub release.
...
Now consider a small voter file with one Census-listed surname (`Smith`) and two surnames that are not in the Census list (`Rajasekhar`, a Tamil name, and `Schwarzenegger`, an Austrian-German name):
...
The two unmatched surnames receive the *same* prediction; standard BISG cannot distinguish them. With eBISG:
...
`Smith` (matched) is unchanged. `Rajasekhar` is now correctly tilted toward Asian (~41%) and `Schwarzenegger` toward White (~99%), reflecting the name-specific signal the embedding model recovers.
...
To use a different sentence-transformer (for example, one of the newer top entries on the [MTEB leaderboard](https://huggingface.co/spaces/mteb/leaderboard)), pass a list to `ebisg.model` with the HuggingFace model ID and a path to a matching MLP checkpoint trained against that encoder:
...
For methodological details, see Dasanaike and Imai (2026), "Embedding-Supplemented Bayesian Improved Surname Geocoding."

### Using name lists (lBISG)

`lBISG` (list-powered BISG) predicts race when no Census surname table is available, such as for finer national-origin or ancestry groups, for religion, or for countries outside the US. It uses no name-by-race frequency table. Instead it takes short, approximate **name lists** for the groups of interest, compiled from expert knowledge or generated by a language model. It treats list membership as a proxy prediction task over name embeddings and corrects it using the geographic prior from the census. A few hundred names per group are usually enough.

`lBISG` is opt-in. It shares eBISG's Python dependencies (`sentence-transformers` and `torch`) and also uses `scikit-learn` and `scipy`. Run `setup_lbisg()` once. No weights are downloaded: the list-membership model is trained at prediction time from the lists you supply, so geolocation (`census.geo`) is required. Supply `lists` as a named list keyed by race abbreviation (`whi`, `bla`, `his`, `asi`); `oth` is the residual and needs no list.
...
The output has the same `pred.whi`/`pred.bla`/`pred.his`/`pred.asi`/`pred.oth` columns as standard BISG.

For methodological details, see Chasalow, Dasanaike and Imai, "Probabilistic Race and Ethnicity Prediction Using Group-Specific Name Lists."
