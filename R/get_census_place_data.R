...
#'
#' \code{get_census_place_data} returns place-level Census data 
...
#' \code{"county"}, \code{"tract"}, or \code{"block"} are supported. Use \code{"place"} for this version
...
get_census_place_data <- function(key, states, age = FALSE, sex = FALSE, census.geo = "place", retry = 0) {
  ...
  for (s in states) {
    ...
    if (census.geo == "place") {
      place <- census_geo_place_api(key, s, geo = "place", age, sex, retry)
