...
get_census_data_combined <- function(key, states, age = FALSE, sex = FALSE, census.geo = "block", retry = 0) {
  ...
  for (s in states) {
    ...
    if (census.geo == "place") {
      place <- census_geo_combined(key, s, geo = "place", age, sex, retry)
      CensusObj[[s]]$place <- place
    }
    if (census.geo == "block") {
      block <- census_geo_combined(key, s, geo = "block", age, sex, retry)
      CensusObj[[s]]$block <- block
    }
    if ((census.geo == "block") || (census.geo == "tract")) {
      tract <- census_geo_combined(key, s, geo = "tract", age, sex, retry)
      CensusObj[[s]]$tract <- tract
    }
    if ((census.geo == "block") || (census.geo == "tract") || (census.geo == "county")) {
      county <- census_geo_combined(key, s, geo = "county", age, sex, retry)
