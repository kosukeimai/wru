...
#'
#' @export
census_helper_combined <- function(key, voter.file, states = "all", geo = "tract", age = FALSE, sex = FALSE, census.data = NA, retry = 0) {
  ...
  for (s in 1:length(states)) {
    ...
    if (geo == "place") {
      ...
      if ((toDownload) || (is.null(census.data[[state]])) || (census.data[[state]]$age != age) || (census.data[[state]]$sex != sex)) {
        census <- census_geo_combined(key, state, geo = "place", age, sex, retry)
      } else {
    ...
    if (geo == "county") {
      ...
      if ((toDownload) || (is.null(census.data[[state]])) || (census.data[[state]]$age != age) || (census.data[[state]]$sex != sex)) {
        census <- census_geo_combined(key, state, geo = "county", age, sex, retry)
      } else {
    ...
    if (geo == "tract") {
      ...
      if ((toDownload) || (is.null(census.data[[state]])) || (census.data[[state]]$age != age) || (census.data[[state]]$sex != sex)) {
        census <- census_geo_combined(key, state, geo = "tract", age, sex, retry)
      } else {
    ...
    if (geo == "block") {
      ...
      if ((toDownload) || (is.null(census.data[[state]])) || (census.data[[state]]$age != age) || (census.data[[state]]$sex != sex)) {
        census <- census_geo_combined(key, state, geo = "block", age, sex, retry)
      } else {
