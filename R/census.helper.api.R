...
#'
#' \code{census.helper.api.online} links user-input dataset with Census data.
#'
...
#' Census helper function.
#'
#' \code{census.helper.api.local} links user-input dataset with Census data.
#'
#' This function allows users to link their geocoded dataset (e.g., voter file) 
#' with U.S. Census 2010 data. The function extracts Census Summary File data 
#' at the tract or block level using the 'UScensus2010' package. Census data 
...
#' 
#' @export
census.helper.api.local <- function(voters, state = NA, geo = "tract", demo = FALSE, censusData = NA) {

  if (is.na(censusData)) {
    stop('Without pre-downloaded census data, please use census.helper.api to access http://api.census.gov/data/key_signup.html.')
  } else {
    if ((censusData$state != state) | (censusData$demo != demo)) {
  ...
  
  {
    ...
    if (geo == "county") {
      geo.merge <- c("state", "county")
      census <- censusData$county
    }
    
    if (geo == "tract") {
      geo.merge <- c("state", "county", "tract")
      census <- censusData$tract
    }
    
    if (geo == "block") {
      geo.merge <- c("state", "county", "tract", "block")
      census <- censusData$block
    }
...
census.helper.api <- function(key, voters, states = "all", geo = "tract", demo = FALSE, censusData = NA) {
  ...
  if (is.na(censusData)) {
    ...
    if ((censusData$state != states) | (censusData$demo != demo)) {
      toDownload = TRUE
    } else {
      toDownload = FALSE
    }
