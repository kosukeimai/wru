...
#'
#' \code{census_place_helper} links user-input dataset with Census geographic data.
#'
...
#'
#' @export
census_place_helper <- function(key, voter.file, states = "all", geo = "place", age = FALSE, sex = FALSE, census.data = NA, retry = 0) {
  ...
  for (s in 1:length(states)) {
    ...
    if (geo == "place") {
      ...
      if ((toDownload) || (is.null(census.data[[state]])) || (census.data[[state]]$age != age) || (census.data[[state]]$sex != sex)) {
        census <- census_geo_place_api(key, state, geo = "place", age, sex, retry)
      } else {
