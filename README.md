...
names that *are* in the Census list, predictions are identical to
standard BISG.

`eBISG` is opt-in. It depends on Python with `sentence-transformers` and
`torch`; the first call additionally downloads about 17 MB of
pre-trained MLP weights from the package’s GitHub release.

Run this once per machine to set up the environment:

``` r
library(wru)
setup_ebisg()
# Installs sentence-transformers + torch into the 'r-ebisg' virtualenv
...
``` r
predict_race(voter.file = voters, census.geo = "tract", model = "eBISG")
```

To use a different sentence-transformer (for example, one of the newer
top entries on the [MTEB
leaderboard](https://huggingface.co/spaces/mteb/leaderboard)), pass a
list to `ebisg.model` with the HuggingFace model ID and a path to a
matching MLP checkpoint trained against that encoder:

``` r
predict_race(
  voter.file = voters, surname.only = TRUE, model = "eBISG",
  ebisg.model = list(
    transformer  = "your-org/your-encoder",
    dim          = 768L,
    surname_mlp  = "/path/to/your-surname-mlp.pt"
  )
)
```
...
### Using name lists (lBISG)

`lBISG` (list-powered BISG) predicts race when no Census surname table
is available, such as for finer national-origin or ancestry groups, for
religion, or for countries outside the US. It uses no name-by-race
frequency table. Instead it takes short, approximate **name lists** for
the groups of interest, compiled from expert knowledge or generated by a
language model. It treats list membership as a proxy prediction task over
name embeddings and corrects it using the geographic prior from the
census. A few hundred names per group are usually enough.

`lBISG` is opt-in. It shares eBISG’s Python dependencies
(`sentence-transformers` and `torch`) and also uses `scikit-learn` and
`scipy`. Run `setup_lbisg()` once. No weights are downloaded: the
list-membership model is trained at prediction time from the lists you
supply, so geolocation (`census.geo`) is required. Supply `lists` as a
named list keyed by race abbreviation (`whi`, `bla`, `his`, `asi`);
`oth` is the residual and needs no list.

``` r
library(wru)
setup_lbisg()   # run once: installs Python dependencies into the 'r-ebisg' virtualenv

lists <- list(
  whi = c("SMITH", "MILLER", "OLSON", "SCHNEIDER"),      # ... a few hundred surnames per group
  bla = c("WASHINGTON", "JEFFERSON", "BOOKER"),
  his = c("GARCIA", "RODRIGUEZ", "HERNANDEZ"),
  asi = c("NGUYEN", "KIM", "PATEL", "WANG")
)

predict_race(voter.file = voters, census.geo = "tract", model = "lBISG", lists = lists)
```

The output has the same
`pred.whi`/`pred.bla`/`pred.his`/`pred.asi`/`pred.oth` columns as
standard BISG. For methodological details, see Chasalow, Dasanaike and
Imai, “Probabilistic Race and Ethnicity Prediction Using Group-Specific
Name Lists.”
